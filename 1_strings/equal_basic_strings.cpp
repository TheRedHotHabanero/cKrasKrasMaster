...
1. сделаете ли вы этот оператор методом класса или собственной функцией?
(придется классом видимо)
2. как должны сравниваться строки с одинаковым CharT, но разными Traits? (никак)
3. а если у них одинаковые CharT и Traits, но разные аллокаторы? (фиолетово,
сравниваем содержимое) 4. "hello" == str str == "hello" (using previous) str2 ==
str1 "hello" == "aaaaa" -> std::strcmp
*/
...
bool operator==(
    const std::basic_string<CharT, Traits, Allocator1> &lhs,
...
int main() {
    ...
    if (help::operator==(str1,
                         "hello")) { // ambiguous overload for ‘operator==’
